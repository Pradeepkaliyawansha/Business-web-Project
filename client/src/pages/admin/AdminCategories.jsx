...
export default function AdminCategories() {
  ...
  const openModal = (cat = null) => {
    setEditTarget(cat);
    setForm(cat ? { name: cat.name, description: cat.description || '', icon: cat.icon || '📦', isActive: cat.isActive } : defaultForm);
    setModalOpen(true);
  };

  const closeModal = () => { setModalOpen(false); setEditTarget(null); setForm(defaultForm); };
  ...
  return (
    <div className="animate-fade-in max-w-4xl">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-display font-bold text-3xl text-white">Categories</h1>
      ...
      {loading ? (
        ...
        <div className="card p-16 text-center">
          <Tag className="w-12 h-12 text-dark-400 mx-auto mb-3" />
          <p className="text-gray-400 mb-4">No categories yet</p>
          <button onClick={() => openModal()} className="btn-primary">Add First Category</button>
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    className="p-1.5 rounded-lg bg-dark-600 hover:bg-primary-500/10 text-gray-400 hover:text-primary-400 border border-dark-500 hover:border-primary-500/30 transition-all"
                  >
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-dark-800 border border-dark-600 rounded-2xl p-6 w-full max-w-md animate-scale-in">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div>
                <label className="text-sm text-gray-400 font-medium block mb-2">Category Name *</label>
                <input
                  type="text"
                  value={form.name}
              ...
              <div>
                <label className="text-sm text-gray-400 font-medium block mb-2">Icon</label>
                  ...
                  {emojis.map((emoji) => (
                    <button
                      ...
                      className={`w-9 h-9 rounded-lg text-xl flex items-center justify-center transition-all ${
                        form.icon === emoji ? 'bg-primary-500/20 border-2 border-primary-500' : 'bg-dark-700 border border-dark-600 hover:border-dark-400'
                      }`}
                    >
                      {emoji}
                    </button>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <div
                  ...
                  className={`w-10 h-5 rounded-full transition-colors ${form.isActive ? 'bg-primary-500' : 'bg-dark-500'} relative cursor-pointer flex-shrink-0`}
                >
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={saving} className="btn-primary flex-1 py-2.5 text-sm flex items-center justify-center gap-2">
                  {saving ? <><div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" /> Saving...</> : (editTarget ? 'Update' : 'Create')}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-dark-800 border border-dark-600 rounded-2xl p-6 max-w-sm w-full animate-scale-in">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="font-display font-semibold text-white">Delete Category?</h3>
            </div>
            <p className="text-sm text-gray-400 mb-6">
              Delete <span className="text-white font-medium">"{deleteConfirm.name}"</span>? Products in this category won't be deleted but may lose their category.
            </p>
